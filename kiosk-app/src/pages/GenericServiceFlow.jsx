...
export default function GenericServiceFlow() {
  const { serviceId, actionId } = useParams()
  ...
  // Find a matching translation or fallback to formatted string
  const getTitle = () => {
    if (t(actionId)) return t(actionId);
    if (!actionId) return 'Service Request';
  ...
  return (
    <section className="flex-1 flex flex-col items-center h-[calc(100vh-5.5rem)] bg-surface px-6 py-8 overflow-hidden">
      ...
      <div className="w-full max-w-5xl flex items-center justify-between mb-8">
        ...
        >
          <ArrowLeft className="w-6 h-6" /> {t('back') || 'Back'}
        </motion.button>
        ...
        <div className="flex items-center gap-4 bg-white px-6 py-3 rounded-xl shadow-sm border border-gray-100">
          ...
          <h2 className="text-2xl font-display font-bold text-gray-800 tracking-tight">{getTitle()}</h2>
        </div>
      </div>
